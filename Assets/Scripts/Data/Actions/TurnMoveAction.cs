...
public class TurnMoveAction : IAction
{
	private readonly int2 delta;

	public TurnMoveAction(int dx, int dy)
	{
		this.delta = new int2(dx, dy);
	}

	public TurnMoveAction(int2 delta)
	{
		this.delta = delta;
	}
	...
	public void Perform(GameObject actor)
	{
		...
		position.Rotation = position.GetClosestAbsoluteDirection(position.Value + delta, 4);
